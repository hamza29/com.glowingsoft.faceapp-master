...
import com.makemeold.agefaceeditor.jabistudio.androidjhlabs.filter.InvertFilter;
...
public class InvertFilterActivity extends SuperFilterActivity implements OnClickListener {
    ...
    public void onClick(View view) {
        ...
        setModifyView(new InvertFilter().filter(AndroidUtils.drawableToIntArray(this.mModifyImageView.getDrawable()), width, height), width, height);
        this.mApplyCount++;
        this.mApplyText.setText(new StringBuilder(APPLY_STRING).append(this.mApplyCount).toString());
    }
}
